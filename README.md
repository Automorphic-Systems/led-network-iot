# sunshine-led-main
Architecture artifacts for Sunshine - a platform aimed at programming large scale ambient LED installations.  It is complemented with Redcloud, an architecture designed to support these types of installations.

# What This Repository Contains.

- Artifacts related to LED effects and nodes
- Deployment resources and scripts where applicable
- Any other non-software based artifacts that require version control

# Vision.

The idea behind Redcloud / Sunshine is to take physical, real world input and translate it to interaction with a network collection of LED systems. Rather than being a spectator to an LED installation, participants in an immersive project are explicitly a part of it just by being in its presence. Important design features include ease of connectivity, horizontal scaling, and programmability of operations.  The resulting network allows for the creation of immersive and ambient systems at scale, and allowing artists flexibility over how interactive LED systems will behave in an installation.  To do this seamless introduces a number of technical challenges that parallel the typical challenges associated with LED installations.

With Redcloud / Sunshine, designers are able to build out large scale interactive LED systems, and with defining LED visualizations made substantially easier, the locus of the work will shift to the "front-end" i.e. working with artists to set up sensors or other feedback-generating mechanisms in their art/workpieces.  With the ability to scale the system while limiting some of the hardware issues intrinsic to LEDs, larger interactive systems can be constructed with greater stability. 

# Design Goals.

There are four design goals that shape this endeavor, roughly in this order:

- Physical world capture : Redcloud / Sunshine will capture data from the physical world, and cause it to perform some meaningful operation on an LED display.  The input source
...
- Near real time response: Redcloud will capture data, operate on it by way of Sunshine, and send it to the LED display with minimal latency.   		
